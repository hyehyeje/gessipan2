...
const TodoMain = ( ) => {
    ...
    let classname = 0; // class 이름을 위한 변수
                    ...
                    <GridDiv>
                        ...
                        <Grid>
                            {data.map((isColored, index) => {
                                const className = `cell-${classname++}`; // 각 셀에 고유한 className 부여
                                const isToday = index === data.length - 1; // 마지막 셀을 오늘 날짜로 설정
                                
                                return isColored ? (
                                    <ColoredCell key={index} color="#4A90E2" className={className} />
                                ) : (
                                    <Cell key={index} className={isToday ? `today-${className}` : className} />
                                );
                            })}
                        </Grid>
                    </GridDiv>
